...
def predict_model(frame, model, target_classes, confidence_threshold):
    ...
    return output, process_time

def detect_and_save_video(video_path, output_path, tmp_path, models, target_classes, confidence_threshold=0.3):
    ...
    model_total_times = [0.0] * len(models)
    start_time = time.time()
    ...
    with ThreadPoolExecutor(max_workers=len(models)) as executor:
        while cap.isOpened():
            ...
            for i, future in enumerate(futures):
                outputs, process_time = future.result()
                model_total_times[i] += process_time
    ...
    print(f"Processing duration: {video_duration:.2f} seconds")
    for i, model_time in enumerate(model_total_times):
        print(f"Model {i + 1} total processing time: {model_time:.2f} seconds")
...
def main():
    ...
    while True:
        ...
        if next_file:
            ...
            detect_and_save_video(video_path, PROCESSED_TS_PATH, TEMP_OUTPUT_PATH, models, TARGET_CLASSES, CONFIDENCE_THRESHOLD)

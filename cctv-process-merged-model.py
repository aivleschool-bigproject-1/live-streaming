...
MODEL_PATH = 'firewithperson_nano.pt'
...
def predict_model(frame, model, target_classes, confidence_threshold):
    try:
        ...
        return [], {cls: 0 for cls in target_classes}
...
def maintain_max_files(directory, max_files, ext_media_sequence):
    try:
        ...
        print(f"Error maintaining max files: {e}")
        return ext_media_sequence
    ...
    try:
        ...
        while cap.isOpened():
            ...
            if frame_count % 2 == 0:  # Process every second frame
                outputs, class_counts = predict_model(frame, model, target_classes, 0.65)
                frame_detections = outputs
                frame_class_counts = class_counts
        ...
        print(f"Error in detect_and_save_video: {e}")
...
def get_next_file_to_process(process_path):
    try:
        ...
        print(f"Error getting next file to process: {e}")
        return None
...
def update_m3u8(ts_file, ts_duration, m3u8_filename, config):
    try:
        ...
        print(f"Error updating m3u8: {e}")
        return config['extinf_max']
    ...
    try:
        ...
        while True:
            ...
            if next_file:
                ...
                detect_and_save_video(video_path, processed_ts_path, TEMP_OUTPUT_PATH, model, target_classes, config)
